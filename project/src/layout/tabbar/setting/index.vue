...
const logout = async () => {
    ...
    //第二件事情:仓库当中关于用于相关的数据清空[token|等信息]
    //第三件事情:跳转到登录页面
...
const changeDark = () => {
    ...
    let boderS = document.getElementsByTagName('body')[0].style;
    //判断HTML标签是否有类名dark
    if (dark.value) {
        html.className = 'dark';
        boderS.setProperty('--top-buttom-boder', 'rgb(76, 77, 79)');
    } else {
        html.className = '';
        boderS.setProperty('--top-buttom-boder', 'rgb(220, 223, 230)')
    }
}
